...
public class ClientEventProducerServiceImpl implements ClientEventProducerService {
    ...
    public void sendEvent(ClientRequestEvent event) {
        try {
            kafkaTemplate.send(properties.topics().out(), event.getUserId(), event).get();
            log.info(INFO_SEND_MESSAGE_KAFKA.getValue(), properties.topics().out(), event.getUserId(), event);
        } catch (Exception e) {
